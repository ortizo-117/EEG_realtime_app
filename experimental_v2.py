...
class Graph:
    ...
    def __init__(self, board_shim):
        ...
        # variables to store the absolute time for streaming
        self.all_data = []  # Will hold (timestamp, sample_vector) tuples
        #self.recording_start_ts = None  # Will store the absolute timestamp at stream start
    ...
    def start_recording(self):
        try:
            ...
            init_data = self.board_shim.get_current_board_data(1)
            ts_channel = BoardShim.get_timestamp_channel(self.board_id)
            self.recording_start_ts = init_data[ts_channel][-1]
            print(self.recording_start_t)
            #if init_data.shape[1] == 0:
             #   logging.warning("No data yet when starting — delaying timestamp initialization.")
    ...
    def trigger_prot(self):
        ...
        if self.recording_start_ts is None:
            logging.warning("Trigger ignored — timestamp base not ready.")
            return
    ...
    def _extract_epoch(self, elapsed):
        ...
        #new code here
        trigger_ts = self.recording_start_ts + elapsed
        # Find nearest timestamp in all_data
        closest_idx = min(range(len(self.all_data)), key=lambda i: abs(self.all_data[i][0] - trigger_ts))
        start_idx = closest_idx - pre_samples
        end_idx = closest_idx + post_samples
        ...
        if start_idx < 0 or end_idx > len(self.all_data):
            logging.warning(f"Not enough data around trigger at {elapsed:.2f}s "
                            f"for ERP extraction (start_idx={start_idx}, end_idx={end_idx}).")
            return


        # Extract data: channels x epoch_length
        #epoch = full_data[np.ix_(self.exg_channels, range(start_idx, end_idx))]
        epoch = np.array([s[1] for s in self.all_data[start_idx:end_idx]]).T  # shape: channels x samples
    ...
    def update(self):
        ...
        #data = self.board_shim.get_current_board_data(self.num_points)
        #timestamps = self.board_shim.get_current_board_data(self.num_points)[BoardShim.get_timestamp_channel(self.board_id)]

        for i in range(data.shape[1]):
            ts = timestamps[i]
            sample = data[:, i].copy()
            self.all_data.append((ts, sample))

        # Optionally, limit memory usage by trimming old samples:
        if len(self.all_data) > 100000:  # adjust for your needs
